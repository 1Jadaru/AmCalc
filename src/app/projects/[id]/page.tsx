...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      setError(null);
      try {
        const res = await makeAuthenticatedRequest(`/api/projects/${projectId}`);
        if (!res.ok) {
  ...
  }, [user, projectId, makeAuthenticatedRequest]);
  ...
  const downloadCSV = (scenario: Scenario) => {
    if (!scenario.amortizationSchedule || scenario.amortizationSchedule.length === 0) {
    ...
    const loanSummary = [
      ...
      `Interest Rate: ${formatPercentage(scenario.interestRate)}`,
      `Term: ${scenario.termYears} years`,
      `Payment Frequency: ${scenario.paymentFrequency}`,
      `Monthly Payment: ${formatCurrency(scenario.paymentAmount)}`,
      `Total Interest: ${formatCurrency(scenario.totalInterest)}`,
      `Total Payments: ${formatCurrency(scenario.totalPayments)}`,
    ...
    const csvContent = [
      ...loanSummary,
      headers.join(','),
      ...scenario.amortizationSchedule.map((row: any) => [
        row.paymentNumber,
                ...
                <div key={scenario.id} className="bg-white rounded-lg shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="text-lg font-semibold">{formatPercentage(scenario.interestRate)}</div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="text-sm text-gray-600">Term</div>
                      <div className="text-lg font-semibold">{scenario.termYears} years</div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="text-sm text-gray-600">Payment Frequency</div>
                      <div className="text-lg font-semibold capitalize">{scenario.paymentFrequency}</div>
                    </div>
                  </div>
                  ...
                  {scenario.paymentAmount && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="bg-blue-50 p-4 rounded-lg">
                        ...
                        <div className="text-lg font-semibold text-blue-900">{formatCurrency(scenario.paymentAmount)}</div>
                      </div>
                      {scenario.totalInterest && (
                        <div className="bg-green-50 p-4 rounded-lg">
                          <div className="text-sm text-green-600">Total Interest</div>
                          <div className="text-lg font-semibold text-green-900">{formatCurrency(scenario.totalInterest)}</div>
                        </div>
                      )}
                      {scenario.totalPayments && (
                        <div className="bg-purple-50 p-4 rounded-lg">
                          <div className="text-sm text-purple-600">Total Payments</div>
                          <div className="text-lg font-semibold text-purple-900">{formatCurrency(scenario.totalPayments)}</div>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {scenario.amortizationSchedule && scenario.amortizationSchedule.length > 0 && (
                    <div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full border-collapse border border-gray-200">
                          ...
                          <tbody>
                            {scenario.amortizationSchedule.slice(0, 5).map((row: any, index: number) => (
                        ...
                        {scenario.amortizationSchedule.length > 5 && (
                          <div className="text-center text-sm text-gray-500 mt-2">
                            Showing first 5 payments of {scenario.amortizationSchedule.length} total payments
                          </div>
                        )}
                      </div>
                    </div>
